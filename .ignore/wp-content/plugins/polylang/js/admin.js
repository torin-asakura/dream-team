...
jQuery(function ($) {
  ...
  if (flagListExist) {
    // Create the jQuery UI selectmenu widget for flags list dropdown and return its instance.
    var selectmenuFlagList = initializeSelectmenuWidget(
      $('#flag_list'),
      Object.assign({}, selectmenuOptions, selectmenuFlagListCallbacks)
    )
  ...
  if (langListExist) {
    initializeSelectmenuWidget(
      $('#lang_list'),
      Object.assign({}, selectmenuOptions, selectmenuLangListCallbacks)
    )
  }
  ...
  $('.pll-configure').on('keydown', function (event) {
    if ('Enter' === event.key) {
      event.preventDefault()
