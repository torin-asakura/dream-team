...
jQuery(function ($) {
  ...
  function addLanguage(language) {
    ...
    var languageTrashIconHtml = $('<td />').append(
    ...
    languagesListTable.on('click', 'span[data-language=' + language.locale + ']', function (event) {
      event.preventDefault()
      // Remove line in languages table.
      $(this).parents('tr').remove()
      // Remove input field.
      var languageField = languageFields
        .children('input[value=' + $(this).data('language') + ']')
        .remove()
   ...
   */
  languagesList.on('selectmenuchange', function () {
    hideError()
  })
  /**
   * Bind click event on "Add language" button
   */
  $('#add-language').on('click', function (event) {
    hideError()
    var selectedOption =
      event.currentTarget.form.lang_list.options[event.currentTarget.form.lang_list.selectedIndex]
  ...
  dialog.dialog({
    ...
    open: function (event, ui) {
      ...
      // see template view-wizard-step-languages.php.
      $(this)
        .find('#dialog-language-flag')
        .empty()
        .prepend($('#lang_list').children(':selected').data('flag-html')) // phpcs:ignore WordPressVIPMinimum.JS.HTMLExecutingFunctions.prepend
    },
    buttons: [
      {
