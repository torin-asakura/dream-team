...
;(function ($) {
  ...
  $('.cptui-delete-top, .cptui-delete-bottom').on('click', function (e) {
    ...
    var submit_delete_warning = $('<div class="cptui-submit-delete-dialog">' + msg + '</div>')
      .appendTo('#poststuff')
      .dialog({
  ...
  $('.cptui-taxonomy-submit').on('click', function (e) {
    if ($('.cptui-table :checkbox:checked').length == 0) {
      e.preventDefault()
      var no_associated_type_warning = $(
        '<div class="cptui-taxonomy-empty-types-dialog">' +
          cptui_tax_data.no_associated_type +
          '</div>'
      )
        .appendTo('#poststuff')
        .dialog({
