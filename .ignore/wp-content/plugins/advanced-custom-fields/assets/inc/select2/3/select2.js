  ...
  function ajax(options) {
    ...
    return function (query) {
      ...
      timeout = window.setTimeout(function () {
        ...
        url =
          typeof url === 'function' ? url.call(self, query.term, query.page, query.context) : url
  ...
  AbstractSelect2 = clazz(Object, {
              ...
              more
                ...
                .html(
                  self.opts.escapeMarkup(
                    evaluate(self.opts.formatLoadMore, self.opts.element, page + 1)
                  )
                )
              window.setTimeout(function () {
    ...
    updateResults: function (initial) {
      ...
      function postRender() {
        ...
        self.positionDropdown()
        if (
          results.find('.select2-no-results,.select2-selection-limit,.select2-searching').length
        ) {
          self.liveRegion.text(results.text())
        } else {
          self.liveRegion.text(
            self.opts.formatMatches(
              results.find('.select2-result-selectable:not(".select2-selected")').length
            )
          )
        }
      }
      ...
      if (opts.formatSearching && this.findHighlightableChoices().length === 0) {
        render(
          "<li class='select2-searching'>" + evaluate(opts.formatSearching, opts.element) + '</li>'
        )
      }
  ...
  MultiSelect2 = clazz(AbstractSelect2, {
    ...
    postprocessResults: function (data, initial, noHighlightUpdate) {
      ...
      if (
        ...
        if (
          !data ||
          (data && !data.more && this.results.find('.select2-no-results').length === 0)
        ) {
