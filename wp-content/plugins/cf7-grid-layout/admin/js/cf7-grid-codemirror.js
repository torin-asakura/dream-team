;(function ($, cme, jscme, csscme) {
  ...
  const $codemirror = $('#cf7-codemirror'),
    ...
    $topTags = $('#top-tags'),
    $bottomTags = $('#bottom-tags')

  let eh = $formEditor.height(),
    ew = $formEditor.width()
  ...
  window.onerror = function (msg, source, lineno, colno, error) {
    ...
    if ($loadScreen.is(':visible')) {
      $loadScreen.html(
        '<div class="js-error"><p><span class="message">' +
          cf7sgeditor.jserror +
  ...
  CodeMirror.defineMode('shortcode', function (config, parserConfig) {
    let scOverlay = {
      token: function (stream, state) {
        ...
        while (
          stream.next() != null &&
          !stream.match(/^\[([a-zA-Z0-9_]+)\*?\s?/, false) &&
          !stream.match(/data-([a-z0-9_]+)/, false)
        ) {}
        return null
      },
    }
  ...
  $(document).ready(function () {
    ...
    $wpcf7Editor.on('grid-ready', function () {
      ...
      $editorTabs.tabs({
          ...
          switch (gridTab) {
            case '#cf7-editor-grid': //grid editor.
              ...
              if (codemirrorUpdated) {
                ...
                //try to set the focus on the 'cf7sgfocus element'
                const $focus = $('.cf7sgfocus', $grid)
                if ($focus.length > 0) {
                  const scrollPos =
                    $focus.offset().top - $(window).height() / 2 + $focus.height() / 2
            ...
            case '#cf7-codemirror': //HTML editor.
              const cursor = cme.getSearchCursor('cf7sgfocus', CodeMirror.Pos(cme.firstLine(), 0), {
                caseFold: true,
                multiline: true,
              })
            ...
            case '#cf7-js-codemirror': //js editor.
              ...
              if (
                $('.container.cf7sg-collapsible', $form).not(
                  '.cf7sg-slider-section > .cf7sg-collapsible'
                ).length > 0
              ) {
                ...
                while (null != match && match.length > 2) {
                  switch (match[1].replace('*', '')) {
                    ...
                    default:
                      ...
                      if ($field.is('.container.cf7-sg-table .field'))
                        formFields[match[2]] += '_row'
        ...
        create: function (e) {
          ...
          $('.ui-tabs-panel:not(#cf7-editor-grid)', $editorTabs).hover(
            disableBodyScroll,
            enableBodyScroll
          )
      ...
      $.fn.createNewCMEditor = function (activate = false) {
        ...
        $this.each(function (activate) {
          ...
          switch (ref) {
            case '#cf7-js-codemirror':
              ...
              $('a.helper', $jsTags).each(function () {
                let $helper = $(this),
                  text = regex.exec($helper.data('cf72post'))
                if ('undefined' != typeof text[2]) {
                  $helper
                    .parent()
                    .append(
                      '<span style="position:relative"><span class="display-none">' +
                        text[2] +
                        '</span></span>'
                    )
                }
          ...
          cm.setSize('100%')
          $cm.on('cf7sg-screen-resize', function () {
            cm.refresh()
          })
    ...
    $('form#post').submit(function (event) {
      ...
      let $embdedForms = '',
        $formNoEmbeds = '',
        codeMirror = ''
      if ('#cf7-editor-grid' == gridTab) {
        //set up the code in the cf7 textarea
        const $txta = $('textarea#wpcf7-form')
        $txta.html($txta.val() + '\n')
        codeMirror = html_beautify($grid.CF7FormHTML(), {
          indent_size: 2,
          wrap_line_length: 0,
        })
      ...
      const cf7TagRegexp = /\[(.[^\s]*)\s*(.[^\s]*)(|\s*(.[^\[]*))\]/gim
      ...
      if (
        $('.cf7sg-collapsible', $formNoEmbeds).not('.cf7sg-slider-section > .cf7sg-collapsible')
          .length > 0
      ) {
      ...
      if (
        codeMirror.indexOf('[date') > 0 ||
        0 < codeMirror.search(/\[text([^\]]+?)class:datepicker/gi)
      )
        scriptClass += 'has-date,'
      ...
      $this.append(
        '<input type="hidden" name="cf7sg-script-classes" value="' + scriptClass + '" />'
      )
      ...
      if (
        ...
        wpcf7cf.getnumberOfFieldEntries() > 0
      ) {
   ...
   */
    $.fn.CF7FormHTML = function () {
      ...
      $('.container.cf7sg-collapsible', $form).each(function () {
        const $this = $(this),
          cid = $this.attr('id'),
          $title = $this.children('.cf7sg-collapsible-title')
      ...
      $('.columns', $form).each(function () {
        ...
        if ($text.length > 0) {
          ...
          if ($toggle.length > 0) {
            ...
            if ($field.length > 0) {
              if (
                $field.is('.checkbox.required') ||
                $field.is('.radio') ||
                $field.is('.file.required')
              )
                isToggled = true
