;(function ($) {
  ...
  $(document).ready(function () {
    $('span.mailtag.code', '#wpcf7-mail').each(function () {
      ...
      var $help = $(
        '<a class="helper" data-tag="' +
          tag +
          '" href="javascript:void(0);"><span class="dashicons dashicons-info"></span></a>'
      )
      $(this).after($help)
      new Clipboard($help[0], {
        text: function (trigger) {
          ...
          var tag = $(trigger).data('tag'),
            ftag = tag.replace(/\-/g, '_')
          var addTable = false
          ...
          var text =
            "add_filter( 'cf7sg_mailtag_" + tag + "', 'filter_cf7_mailtag_" + ftag + "', 10, 3);\n"
          ...
          if (addTable) {
            text +=
              "    $style = 'style=[qtm]padding:0 3px;border-collapse:collapse;border-bottom:1px solid black[qtm]';\n"
            text += "    $tag_replace ='\n"
            text += '    <table>\n'
            text +=
              "      <thead><tr><th '.$style.'>Guest Name</th><th '.$style.'>Senior</th></tr></thead>\n"
            text += "      <tbody>';\n"
