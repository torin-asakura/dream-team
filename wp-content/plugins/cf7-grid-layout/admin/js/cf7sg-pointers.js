  ...
  function createPointer(id, pointer) {
    ...
    var pointer = $point
      .pointer({
        ...
        pointerWidth: 350,
        close: function () {
          $.post(ajaxurl, { pointer: id, action: 'dismiss-wp-pointer' })
        },
      })
      .pointer('open')
    $('.wp-pointer-content .cf7sg-pointer')
      .parent()
      .find('a.close')
      .after(
        '<a href="#" data-id="' +
          id +
          '" class="button-primary cf7sg-next">' +
          cf7sg_pointers.next +
          '</a>'
      )
    return pointer
