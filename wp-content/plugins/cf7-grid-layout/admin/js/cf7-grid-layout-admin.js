;(function ($) {
  ...
  const $pattern = $('<div>').html(
      cf7grid.preHTML + '\\s*(\\[.*\\s*\\].*\\s*){0,}\\s*' + cf7grid.postHTML
    ),
  ...
  let seekTemplate = false,
    cssTemplate = 'div.field',
    ...
    */
    function buildGridForm() {
      ...
      if (0 === formhtml.length) {
        formhtml =
          '<div class="container"><div class="row"><div class="columns full"></div></div></div>'
      }
      ...
      $('div.container.cf7sg-collapsible', $form).each(function () {
        ...
        const $ctrl = $this
          .children('.row')
          .children('.row-controls')
          .find('.collapsible-row-label')
        $('input', $ctrl).prop('checked', true)
        //toggle disable the sibling input
        $('input', $ctrl.siblings('.unique-mod')).prop('disabled', function (i, v) {
          return !v
        })
      })
      /*--------------------------------------------------- convert tables */
        ...
        if ($('#grid-cf7-forms .form-select option[value="' + id + '"]').length > 0) {
          ...
          $.post(ajaxurl, data, function (response) {
            if (response.length > 0) {
              $('.cf7sg-external-form-content', $extform)
                .attr('id', 'cf7sg-form-' + id)
                .append(response)
      ...
      $('.cf7sg-accordion-rows.columns, .cf7sg-slider-section.columns', $form).each(function () {
        ...
        if ($col.is('.cf7sg-accordion-rows')) {
          ...
          //is .cf7sg-slider-section
          $('.slider-rows:input', $control).prop('checked', true)
        }
        //remove toggle checkbox.
        $(
          'input[type="checkbox"]',
          $col.children('.cf7sg-collapsible').children('.cf7sg-collapsible-title')
        )
          .hide()
          .next('span')
          .hide()
      ...
      $('.row .cf7sg-collapsible:first-child', $form).each(function () {
        $(this)
          .siblings('.cf7sg-collapsible')
          .closest('.columns')
          .children('.grid-column')
          .addClass('enable-grouping')
      })
      //add the form to the grid
      if (
        $form.children('.container').length > 0 ||
        $form.children('.cf7sg-external-form').length > 0
      ) {
        $grid.append($form.children())
      } else {
        //this is not a cf7sg form.
        $grid.html($form.html())
      }
    ...
    $grid.on('change', function (event) {
      ...
      if ($target.is('.column-setting')) {
        //----------- column size/offset settings
        ...
        const $column = $target.closest('.columns'),
          classList = $column.attr('class').split(/\s+/)
      ...
      } else if ($target.is('.form-select')) {
        //-------------- external form selection
        ...
        $.post(ajaxurl, data, function (response) {
          $('.cf7sg-external-form-content', $container)
            .attr('id', 'cf7sg-form-' + $target.val())
            .html(response)
        })
      }
      /*
      ...
      */
      if ($target.is('.cf7sg-collapsible-title label input[type="text"]')) {
        //------- Collapsible title
        $target.siblings('input[type="hidden"]').val($target.val())
      } else if ($target.is('.cf7sg-collapsible-title label input[type="checkbox"]')) {
        //------- Collapsible title toggled
        const $title = $target.closest('.cf7sg-collapsible-title')
        if ($target.is(':checked')) {
          $title.append($('#grid-collapsible-with-toggle').html())
          $title
            .closest('.container.cf7sg-collapsible')
          ...
          $title
            .closest('.container.cf7sg-collapsible')
            .removeClass('with-toggle')
            .removeAttr('data-group')
            .removeAttr('data-open')
        }
      } else if ($target.is('ul.cf7-sg-tabs-list li label input[type="text"]')) {
        //------- Tabs title
    ...
    $grid.on('click', function (event) {
      ...
      */
      ...
      if ($target.is('.dashicons-trash.form-control')) {
      ...
      } else if ($target.is('.dashicons-plus.row-control')) {
        //-----------ADD Row
        $target.closest('.container').insertNewRow()
      } else if ($target.is('.dashicons-edit.row-control')) {
        //-----------Show controls
        ...
        */
      } else if ($target.is('.dashicons-no-alt.row-control')) {
        //----------------hide controls
        //take care by closeAllControls
      } else if ($target.is('input.collapsible-row')) {
        //-------------checkbox collapsible row
        ...
        $target
          .parent()
          .siblings('label.unique-mod')
          .children('input')
          .prop('disabled', function (i, v) {
            return !v
          })
      } else if ($target.is('input.table-row')) {
        //-------------checkbox table row
        if ($target.is(':checked')) {
          ...
          $target
            .closest('.container')
            .addClass('cf7-sg-table')
            .attr('id', id)
            .fireGridUpdate('add', 'table-row')
        } else {
          $target.closest('.row').removeClass('cf7-sg-table')
          $target
            .closest('.container')
            .removeClass('cf7-sg-table')
            .removeAttr('id')
            .removeAttr('data-button')
            .fireGridUpdate('remove', 'table-row')
        }
        //toggle disable the sibling input
        $target
          .parent()
      ...
      } else if ($target.is('input.tabs-row')) {
        //-------------checkbox tabbed row
      ...
      } else if ($target.is('input.footer-row')) {
        //-------------checkbox footer row
        ...
        if ($table.is('.container.cf7-sg-table')) {
          if ($target.is(':checked')) {
            $target
              .closest('.container')
              .addClass('cf7-sg-table-footer')
              .fireGridUpdate('add', 'footer-row')
          } else {
            $target
              .closest('.container')
              .removeClass('cf7-sg-table-footer')
              .fireGridUpdate('remove', 'footer-row')
          }
          //toggle disable the sibling input
          $target
            .parent()
      ...
      } else if ($target.is('.make-grid.row-control')) {
        /** @since 3.4.0--wrap row into column ->make grid*/
      ...
      */
      ...
      if ($target.is('.dashicons-edit.column-control')) {
        //------------------show controls
        //now show this control
        $target.siblings('.grid-controls').show().filterColumnControls()
        $target.hide()
        $target.siblings('.dashicons-no-alt').show()
      } else if ($target.is('.php-icon.column-control')) {
        //--------show hooks
      ...
      } else if ($target.is('.js-icon.column-control')) {
        //--------show js hooks
      ...
      } else if ($target.is('.dashicons-trash.column-control')) {
        //-------------------delete column
        $parentColumn.closest('.row').fireGridUpdate('remove', 'column')
        $parentColumn.remove()
        //refilter
        //$target.siblings('.grid-controls').filterColumnControls();
      } else if ($target.is('.make-grid.column-control')) {
        //--------------------add row/convert column to grid
      ...
      } else if ($target.is('.external-form')) {
        //---------------- insert cf7 form
        //close the grid-control box
        $target.parent().hide()
        $target.parent().siblings('.dashicons-no-alt').hide()
      ...
      } else if ($target.is('.dashicons-plus.column-control')) {
        //--------------------add column
        $parentColumn = $target.closest('.columns')
        ...
        let classList,
          ...
          total = 0
        let sizes = [],
          $newColumn = $('<div class="columns"></div>')
        ...
        if (12 == total) {
          ...
          if (!createColumn) {
            //not enough space to create extra column
            /*TODO: display an error message and return */
            return false
          }
        } else if (total < 12) {
          //just add the new column as size 1
          if (12 - total < total) {
            newSize = 12 - total - 1
          } else {
            newSize = total - 1
      ...
      } else if ($target.is('.accordion-rows.column-control')) {
        /** @since 3.4.0 enable accordion */
        if ($target.is(':checked')) {
          $parentColumn
            .addClass('cf7sg-accordion-rows')
            .removeClass('cf7sg-slider-section')
            .removeAttr('data-next data-prev data-submit data-dots')
          $target
            .parent('label')
            .siblings('.grouping-option')
            .children(':input')
            .prop('checked', false)
          ...
          $(
            'input[type="checkbox"]',
            $parentColumn.children('.cf7sg-collapsible').children('.cf7sg-collapsible-title')
          )
            .hide()
            .next('span')
            .hide()
      ...
      } else if ($target.is('.slider-rows.column-control')) {
        /** @since 3.4.0 enable slider */
        if ($target.is(':checked')) {
          /** @since 4.7.2 enable dots on sliders */
          let attrs = { 'data-next': '', 'data-prev': '', 'data-submit': '', 'data-dots': 'false' }
          $target
            .closest('.columns')
            .addClass('cf7sg-slider-section')
            .attr(attrs)
            .removeClass('cf7sg-accordion-rows')
          $target
            .parent('label')
            .siblings('.grouping-option')
            .children(':input')
          ...
          $target
            .closest('.columns')
            .removeClass('cf7sg-slider-section')
            .removeAttr('data-next data-prev data-submit')
    ...
    if (cf7grid.ui) {
      $grid.keydown('div.cf7-field-inner', function (e) {
        ...
        if ($target.is('div.cf7-field-inner :input')) {
          ...
          let found = false,
            stopSearch = false,
            $next = $grid.find('div.cf7-field-inner').filter(function (i, el) {
    ...
    $grid.on('cf7grid-form-finalise', function () {
      if (cf7grid.ui) {
        $('#wpcf7-form')
          .parent()
          .siblings('textarea.grid-input')
          .on('change', function () {
            $grid.trigger('cf7grid-form-ready') //codemirror initialisation
          })
        $('textarea.grid-input#wpcf7-form').on('change', function () {
          //special case for custom code in cf7ui mode
          $grid.trigger('cf7grid-form-ready') //codemirror initialisation
        })
        changeTextarea(true)
      } else {
    ...
    $('.row', $grid).sortable({
      ...
      receive: function (event, ui) {
        ...
        if (row.length > 12) {
          ...
          const $warning = $(
            '<span class="cf7sg-warning">Not enough space in this row to add column</span>'
          )
  ...
  $.fn.html2gui = function (html) {
    ...
    if (seekTemplate && 1 == search.children(cssTemplate).length) {
      ...
      if (null !== match) {
        ...
        //this html does not match our templates
  ...
  $.fn.scanCF7Tag = function () {
    ...
    while (match != null && !stopSearch) {
      ...
      tag = match[1].replace('*', '')
      field = match[2]
      let helpers = ['cf7sg-tag-all'],
        jsHelpers = ['cf7sg-tag-all']
      ...
      switch (tag) {
        ...
        case 'save':
          ...
          break
        case 'textarea':
          if (match[0].search(/\s[0-9]{0,3}x[0-9]{1,3}\s?/gi) < 0) {
    ...
    if (count - counth > 1) {
      /** @since 3.2.1 don't count hidden fields */
  ...
  $.fn.getRowSize = function () {
    ...
    $(this)
      ...
      .each(function (index) {
        ...
        if (!foundSize) {
          //by default a colum which is not set set is treated as 1
          sizes[index] = 0
          total += 1
        }
      })
  ...
  $.fn.getColumnTotalSize = function () {
    ...
    let off,
      ...
      total = 0,
      classList = $this.attr('class').split(/\s+/)
    const $sizes = $this.find('.grid-column select.column-size'),
      $offsets = $this.find('.grid-column select.column-offset')
    $offsets.val('')
    $sizes.val('one')
    foundSize = false
    for (let idx = 0; idx < classList.length; idx++) {
  ...
  $.fn.insertNewRow = function (areaCode) {
    ...
    switch (true) {
      ...
      case $this.is('.cf7sg-external-form'):
        ...
        break
      default:
        //unknown element, maybe an error.
        return $this
    }
  ...
  $.fn.filterColumnControls = function () {
    ...
    const $parentRow = $this.closest('.row'),
      $parentColumn = $this.closest('.columns')
    const row = $parentRow.getRowSize(),
      col = $parentColumn.getColumnTotalSize()
    let idx,
      start,
      free = 0
