;(function ($) {
  ...
  var cf7sgCustomSelect2Templates = (function (ccst) {
    return ccst
  ...
  $.fn.cf7sgWarning = function (msg, time) {
    ...
    var $warning = $(
      '<span class="cf7sg-validation-warning">' +
        msg +
        '<span class="confirm-button">ok</span></span>'
    )
    $this.after($warning)
    if (time > 0) {
  ...
  $(document).ready(function () {
    ...
    $('form.wpcf7-form')
      ...
      .each(function () {
        /** @since 4.4 setup form id */ var $form = $(this),
          id = $form.closest('div.cf7-smart-grid').attr('id')
    ...
    if ($cf7Form_table.length) {
      $('.container.cf7-sg-table', $cf7Form_table).each(function () {
        var $table = $(this),
          fid = $table.closest('div.cf7-smart-grid').attr('id')
        ...
        $(':input', $row).each(function () {
          /** @since 4.4 */ var $input = $(this).prop('disabled', true),
            name = '_cf7sgcloned_' + $input.attr('name')
          $input.attr('name', name)
        })
        //add controls to the row to delete
        $row.append(
          '<span class="row-control"><span class="dashicons dashicons-no-alt"></span></span>'
        )
    ...
    if (cf7Form_accordion.length > 0) {
      ...
      cf7Form_accordion.filter('div.has-toggles form.wpcf7-form').each(function () {
        ...
        $form.on('click', '.cf7sg-collapsible.with-toggle', function (e) {
          ...
          var $toggledSection = $header.closest('.container.cf7sg-collapsible'),
            id = $toggledSection.attr('id')

          //close other toggled sections if we have a group.
          var group = $header.parent().removeClass('collapsed').data('group')
          if (group) {
            $('.cf7sg-collapsible.with-toggle[data-group="' + group + '"]', $form).each(
              function () {
                var $toggled = $(this),
                  $togl = $('.toggle', $toggled)
          ...
          if ($header.hasClass('ui-state-active')) {
            ...
            var inputs = $('.row.ui-accordion-content :input', $header.parent())
        ...
        toggled_accordion.each(function () {
          ...
          $section
            .children('.cf7sg-collapsible-title')
            .children('.toggle')
            .setupToggle(toggled, group)
          if (toggled) {
      ...
      cf7Form_accordion.each(function () {
        ...
        var $form = $(this),
          $rows = $('.cf7sg-collapsible', $form)
            .not('.cf7sg-collapsible.with-toggle')
            .not('.cf7sg-accordion-rows > .cf7sg-collapsible')
            .not('.cf7sg-slider-section >.cf7sg-collapsible')
        ...
        $rows.each(function () {
          ...
          if ($row.is('.cf7sg-accordion-rows')) {
            ...
            $row
              .children('.cf7sg-collapsible')
              .children('.cf7sg-collapsible-title')
              .addClass('accordion')
            Object.assign(options, {
              header: 'div.cf7sg-collapsible-title.accordion',
              animate: false,
            })
          } else {
            Object.assign(options, {
    ...
    $form_slider.each(function () {
      ...
      $('.cf7sg-slider-section').each(function () {
        var glider,
          $slider = $(this).wrapInner('<div class="glider"></div>'),
          slideCount = -1,
    ...
    $('div.cf7_2_post div.cf7-smart-grid.has-toggles form.wpcf7-form').each(function () {
      var $form = $(this),
        fid = $form.closest('div.cf7-smart-grid').attr('id')
      var nonceID = $form.closest('div.cf7_2_post').attr('id')
      if (nonceID.length > 0) {
        $form.on(nonceID, function (event) {
     ...
     */
    $('div.cf7-smart-grid')
      .on('wpcf7:invalid wpcf7invalid wpcf7mailsent', '.wpcf7', function (e) {
        var $target = $(e.target),
          invalids
        switch (e.type) {
          case 'wpcf7mailsent' /** @since 4.6.0 success, redirect? */:
            if (
              !isEmpty(cf7sg[e.delegateTarget.id]) &&
              cf7sg[e.delegateTarget.id].redirect.length > 0
            ) {
              ...
              for (var idx in invalids) {
                ...
                for (var sdx = 0; sdx < $section.length; sdx++) {
                  ...
                  if ($($section[sdx]).is('.cf7sg-accordion-rows .cf7sg-collapsible')) {
                    $($section[sdx])
                      .closest('.cf7sg-accordion-rows')
                      .accordion('option', 'active', sdx)
                  } else {
                ...
                if ($section.length > 0) {
                  ...
                  if ($section.data('dots')) {
                    $(
                      '.slider-dots button[data-index="' + $slide.data('gslide') + '"]',
                      $section
                    ).attr('data-cf7sg', 'error')
                  }
                }
              }
            }
    ...
    $('div.cf7-smart-grid.has-grid .wpcf7-submit').each(function () {
      var $submit = $(this),
        fid = $submit.closest('div.cf7-smart-grid').attr('id')
      $submit
        .after('<span class="cf7sg-popup display-none">' + cf7sg[fid].submit_disabled + '</span>')
        .parent()
        .addClass('cf7sg-popup')
    })
    /** enable max rows.
     * @since 2.8.0
     */
    ...
    $('div.cf7-smart-grid.has-table').on(
      'sgRowDeleted',
      '.container.cf7-sg-table',
      function (event) {
        var max,
          row,
          $table = $(this)
  ...
  */
  ...
  $.fn.cf7sgSelect2Options = function () {
    var $select2 = $(this),
      s2options = { tags: $select2.is('.tags') },
      field = $select2.attr('name').replace('[]', '')
  ...
  $.fn.getCF7field = function (name, obj) {
    ...
    var hasTab = 'undefined' != typeof obj.tab,
      hasRow = 'undefined' != typeof obj.row,
      $result = []
    switch (true) {
      ...
      case hasRow && obj.row > 0: //row n on all tabs | single value on row n.
        $result = $(':input[name="' + name + '_row-' + obj.row + '"]', $form).add(
          $(':input[name*="' + name + '_tab-"]', $form).filter(
            ':input[name$="_row-' + obj.row + '"]'
          )
        )
        break
      case hasRow: //row 0 for all tabs | single value in row 0.
        ...
        break
      default:
        //single field in form | all fields in all rows on all tabs.
        $result = $(':input[name*="' + name + '"]', $form)
    }
  ...
  $.fn.toggleCF7sgTableRowDeletion = function (active) {
    ...
    if (!$table.is('.container.cf7-sg-table')) return false
    if (!active)
      $('.row.cf7-sg-table:nth-last-child(2) .row-control', $table).addClass('display-none')
  ...
  $.fn.toggleCF7sgTabDeletion = function (active) {
    ...
    if (!$tab.is('div.cf7-sg-tabs')) return false
    if (!active)
      $('.cf7-sg-tabs-list li:last-child .cf7sg-close-tab', $tab).addClass('display-none').hide()
    else
      $('.cf7-sg-tabs-list li:last-child .cf7sg-close-tab', $tab).removeClass('display-none').show()
    return $tab
  }
  //count tabs.
  ...
  $.fn.cf7sgRemoveTab = function () {
    ...
    if ($tabList.length > 1) {
      ...
      if ($tabList.last().remove().is('.ui-state-active'))
        $tab.tabs({ active: $tabList.length - 2 }) //remove tab
  ...
  $.fn.cf7sgCloneTab = function (initSelect, human) {
    ...
    $('.cf7sg-collapsible.with-toggle', $newPanel).each(function () {
       ...
       */
      var rootId = $section.attr('id'),
        id = rootId + '_tab-' + (tabCount - 1)
      ...
      $('.cf7sg-collapsible', $newPanel).accordion({
        ...
        create: function (e) {
          $(this).trigger({
            type: 'sgCollapsibleRowsReady',
            'section-id': rootId,
            'tab-index': tabCount - 1,
          })
        },
      })
