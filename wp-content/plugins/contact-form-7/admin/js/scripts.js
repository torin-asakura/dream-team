;(function ($) {
  ...
  $(function () {
    ...
    $('#contact-form-editor-tabs')
      .focusin(function (event) {
        $('#contact-form-editor .keyboard-interaction').css('visibility', 'visible')
      })
      .focusout(function (event) {
        $('#contact-form-editor .keyboard-interaction').css('visibility', 'hidden')
      })
    ...
    $('#wpcf7-sendinblue-enable-contact-list, #wpcf7-sendinblue-enable-transactional-email').on(
      'change',
      function () {
        if ($(this).is(':checked')) {
  ...
  wpcf7.toggleMail2 = function (checkbox) {
    ...
    var $fieldset = $('fieldset', $checkbox.closest('.contact-form-editor-box-mail'))
  ...
  wpcf7.updateConfigErrors = function () {
    ...
    $('[data-config-field]').each(function () {
      ...
      if (errors[section]) {
        ...
        $.each(errors[section], function (i, val) {
          ...
          if (val.link) {
            $li.append($('<a></a>').attr('href', val.link).text(val.message))
          } else {
            $li.text(val.message)
          }
          ...
          var tab = section.replace(/^mail_\d+\./, 'mail.').replace(/\..*$/, '')
    ...
    $('#contact-form-editor-tabs > li').each(function () {
      ...
      if (errorCount[tab.replace('-', '_')]) {
        ...
        if (1 < errorCount[tab.replace('-', '_')]) {
          var manyErrorsInTab = wpcf7.configValidator.manyErrorsInTab.replace(
            '%d',
            errorCount[tab.replace('-', '_')]
          )
    ...
    if (errorCount.total) {
      ...
      if (1 < errorCount.total) {
        $warning.append(wpcf7.configValidator.manyErrors.replace('%d', errorCount.total))
      } else {
