    ...
    function reloadPageForEmptyPost(lang) {
      ...
      if (-1 != location.search.indexOf('new_lang')) {
        ...
        window.location.search = window.location.search.replace(
          /(?:new_lang=[^&]*)(&)?(.*)/,
          'new_lang=' + lang + '$1$2'
        ) // phpcs:ignore WordPressVIPMinimum.JS.Window.location, WordPressVIPMinimum.JS.Window.VarAssignment
      } else {
        window.location.search =
          window.location.search +
          (-1 != window.location.search.indexOf('?') ? '&' : '?') +
          'new_lang=' +
          lang // phpcs:ignore WordPressVIPMinimum.JS.Window.location, WordPressVIPMinimum.JS.Window.VarAssignment
      }
    }
