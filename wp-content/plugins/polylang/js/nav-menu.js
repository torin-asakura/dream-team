      ...
      $("input[value='#pll_switcher'][type=text]")
        ...
        .each(function () {
          ...
          for (var i = 0, idsLength = ids.length; i < idsLength; i++) {
            ...
            label = $('<label>')
              .attr('for', 'edit-menu-item-' + ids[i] + '-' + item)
              .text(' ' + pll_data.strings[ids[i]])
            ...
            if (
              (typeof pll_data.val[item] != 'undefined' && pll_data.val[item][ids[i]] == 1) ||
              (typeof pll_data.val[item] == 'undefined' && ids[i] == 'show_names')
            ) {
