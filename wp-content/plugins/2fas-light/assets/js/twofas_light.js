...
;(function ($) {
  ...
  function showTwofasToast(type, content) {
    ...
    notification =
      '<div class="twofas-light-toast js-twofas-light-toast twofas-light-toast-' +
      type +
      '">' +
      '<div class="twofas-light-toast-content"><p>' +
      content +
      '</p></div></div>'
  ...
  function reloadQRCodeConfirmed(e) {
    ...
    jQuery
      ...
      .error(function () {
        showTwofasToast(
          'error',
          sprintf(__("Couldn't reload QR code.%sPlease try one more time!", '2fas-light')),
          '<br />'
        )
      })
      .always(function () {
    ...
    $.ajax({
      ...
      .done(function (response) {
        ...
        showTwofasToast(
          'success',
          sprintf(
            __('Two-factor authentication%shas been configured successfully.', '2fas-light'),
            '<br />'
          )
        )
        hideConfigWrapper()
      ...
      .fail(function (response) {
        if (response.status === 400) {
          ...
          showTwofasToast(
            'error',
            sprintf(__('Token is invalid :(%sPlease try one more time.', '2fas-light'), '<br />')
          )
        } else {
          showTwofasToast(
            'error',
            sprintf(__('Something went wrong.%sPlease try one more time!', '2fas-light'), '<br />')
          )
        }
      })
      .always(function () {
  ...
  function removeTrustedDevice(e) {
    ...
    $.ajax({
      ...
      .done(function (response) {
        ...
        showTwofasToast(
          'success',
          sprintf(__('The trusted browser%shas been removed', '2fas-light'), '<br />')
        )
      })
      .error(function () {
        showTwofasToast(
          'error',
          sprintf(
            __("Couldn't remove web browser/device.%sPlease try one more time!", '2fas-light'),
            '<br />'
          )
        )
      })
    ...
    $.ajax({
      ...
      .done(function (response) {
        ...
        codes.forEach(function (code, index) {
          ...
          $('.twofas-backup-codes-inputs').append(
            '<input type="hidden" name="codes[' + index + ']" value="' + code + '" />'
          )
        })
      ...
      .error(function (e) {
        if (
          ...
          e.responseJSON.error === 'Cannot perform this action because second factor is disabled.'
        ) {
          showTwofasToast(
            'error',
            'Cannot perform this action because <br />second factor is disabled.'
          )
        } else {
  ...
  function totpChangeRequest(action) {
    $.ajax({
      ...
      .error(function () {
        showTwofasToast(
          'error',
          sprintf(__('Something went wrong.%sTry one more time!', '2fas-light'), '<br />')
        )
      })
      .always(function () {
        totpSwitch.attr('disabled', false)
  ...
  function formatTrustedDeviceAddedOnTimestamps() {
    formatDateFromTimestampAttribute(
      trustedDevicesWrapper.find('table tbody [data-timestamp-to-format]')
    )
  }
  ...
  function getObligatoryRoles() {
    return $('input:checkbox[name="role-obligatory"]:checked')
      .map(function () {
        return this.value
      })
      .get()
  ...
  function getTrustedBrowsersRoles() {
    return $('input:checkbox[name="role-trusted"]:checked')
      .map(function () {
        return this.value
      })
      .get()
